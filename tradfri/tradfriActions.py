...
#
# changelog   :
# - v1.1.1      dTLS COAP value                                         (florian)
...
def tradfri_power_light(hubip, securityid, dtlstoken, lightbulbid, value):
    ...
    api = '{} -m put -u "{}" -k "{}" -e \'{}\' "{}"' .format(coap, dtlstoken, securityid,
                                                                          payload, tradfriHub)
...
def tradfri_dim_light(hubip, securityid, dtlstoken, lightbulbid, value):
...
def tradfri_color_light(hubip, securityid, dtlstoken, lightbulbid, value):
...
def tradfri_power_group(hubip, securityid, dtlstoken, groupid, value):
    """ function for power on/off tradfri lightbulb """
...
def tradfri_dim_group(hubip, securityid, dtlstoken, groupid, value):
    """ function for dimming tradfri lightbulb """
    tradfriHub = 'coaps://{}:5684/15004/{}'.format(hubip, groupid)
    dim = float(value) * 2.55
